...
public interface IRuntimeJobBuilder
{
    ...
    /// <param name="options">Configures the <see cref="JobOptionBuilder"/>, like the cron expression or parameters that get passed down.</param>
    void AddJob(Delegate jobDelegate, Action<JobOptionBuilder>? options = null);
    ...
    [Obsolete("This method will be dropped in the next major version. Register untyped jobs using AddJob(Delegate, Action<JobOptionBuilder>?) overload instead.")]
...
internal class RuntimeJobBuilder : IRuntimeJobBuilder
{
    private readonly IJobRegistrator jobRegistrator;

    public RuntimeJobBuilder(IJobRegistrator jobRegistrator)
    {
        this.jobRegistrator = jobRegistrator;
    }
    ...
    {
        jobRegistrator.AddJob(jobType, options);
    }
    ...
    public void AddJob(
        ...
        Action<JobOptionBuilder>? options = null)
    {
        jobRegistrator.AddJob(jobDelegate, options);
    }

    public void AddJob(Delegate jobDelegate,
        string cronExpression,
        TimeZoneInfo? timeZoneInfo = null,
        string? jobName = null)
    {
