...
public class RunDependentJobTests : JobIntegrationBase
{
    ...
    public async Task CorrelationIdIsSharedByJobsAndTheirDependencies()
    {
        ...
        var correlationId = ServiceProvider.GetRequiredService<IInstantJobRegistry>().ForceRunInstantJob<PrincipalCorrelationIdJob>(token: CancellationToken);

        await CommunicationChannel.Reader.ReadAsync(CancellationToken);
        Storage.Entries.Count.ShouldBe(2);
        Storage.Entries.Distinct().Count().ShouldBe(1);
        Storage.Entries.First().ShouldBe(correlationId.ToString());
    }
