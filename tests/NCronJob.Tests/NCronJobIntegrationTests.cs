...
public sealed class NCronJobIntegrationTests : JobIntegrationBase
{
    ...
    public async Task CanRunSecondPrecisionAndMinutePrecisionJobs()
    {
        ...
        await WaitForNthOrchestrationState(events, ExecutionState.OrchestrationCompleted, 61);

        subscription.Dispose();

        Storage.Entries.Count(e => e == "DummyJob - Parameter: Minute").ShouldBe(1);
        Storage.Entries.Count(e => e == "DummyJob - Parameter: Second").ShouldBe(60);
    }
